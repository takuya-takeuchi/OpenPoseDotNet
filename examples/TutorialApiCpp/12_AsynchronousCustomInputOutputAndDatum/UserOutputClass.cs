...
namespace AsynchronousLoopCustomInputAndOutput
{
    ...
    internal sealed class UserOutputClass
    {
        ...
        public bool Display(StdSharedPtr<StdVector<StdSharedPtr<UserDatum>>> datumsPtr)
        {
            try
            {
        ...
        public void PrintKeyPoints(StdSharedPtr<StdVector<StdSharedPtr<UserDatum>>> datumsPtr)
        {
            if (datumsPtr != null && datumsPtr.TryGet(out var data) && !data.Empty)
            {
